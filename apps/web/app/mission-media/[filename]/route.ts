...
export async function GET(_request: Request, { params }: { params: Promise<{ filename: string }> }) {
  ...
  try {
    ...
    return new Response(image, { headers: { 'content-type': TYPES[path.extname(filename).toLowerCase()], 'cache-control': 'public, max-age=3600', 'x-content-type-options': 'nosniff' } });
  } catch (error) {
